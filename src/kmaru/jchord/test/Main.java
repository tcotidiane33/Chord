...
public class Main {
    ...
    private static List<Map<String, String>> partitionData() throws IOException, FileNotFoundException {
    ...
    private static void dropNode() throws Exception {
        ...
        ChordNode nodeToDelete = chord.getNode(index);
        if (nodeToDelete == null) {
            System.out.println("Nœud introuvable à l'index spécifié.");
            return;
        }

        // Trouver le successeur du nœud à supprimer
        ChordNode successor = chord.getSuccessor(nodeToDelete);

        if (successor != null) {
            // Transférer les données au successeur
            transferData(nodeToDelete, successor);

            // Mettre à jour les références
            chord.updateReferences(nodeToDelete, successor);
            ...
            System.out.println("Aucun successeur trouvé pour le nœud à supprimer. Opération annulée.");
        }
    }

    private static void transferData(ChordNode sourceNode, ChordNode destinationNode) {
        // Transférer les données stockées sur le nœud source vers le nœud de destination
        Map<String, String> data = sourceNode.getAllData();
        for (Map.Entry<String, String> entry : data.entrySet()) {
            destinationNode.addData(entry.getKey(), entry.getValue());
        }
    }
